let apiConsultantApi = new TempApi.ConsultantApi();import TempApi from '../src/index';let apiCandidateApi = new TempApi.CandidateApi();let candidate = new TempApi.Candidate();document.getElementById('if2fd').onclick = (event) => {
...
let arrayi6k9i = [];
document.getElementById("izi8a").onclick = event => {
  event.preventDefault();
  const select = document.getElementById("iso6o")
  arrayi6k9i.push({
      value: select.value,
...
function refreshULixaze() {
let e=``;
for (let y=0; y<arrayi6k9i.length; y++)
 {
   e += `<li index='${y}' arrayvalue='${arrayi6k9i[y].value}'><p style="display: inline-block">${arrayi6k9i[y].liValue}</p><button class="btn pointer bi bi-trash delete-btn" style="display: inline-block;float: right;background-color: red;color: white;" index='${y}'>&nbsp;Delete</button></li>`;
 }
document.getElementById("ixaze").innerHTML = e;
}

document.getElementById("ixaze").addEventListener("click", event => {
  event.preventDefault();
  arrayi6k9i = arrayi6k9i.filter(
    (item, index) => +event.target.getAttribute("index") !== index
  );
  refreshULixaze();
});
function initializearrayi6k9i(data) {
  arrayi6k9i = data.map(item => ({
    value: item._id,
    liValue: item['consname']
  }));
}
